...
class Whatsapp extends CI_Controller {
    ...
    private function _recibir_mensaje() {
        ...
        if ($this->Whatsapp_model->mensaje_existe($wa_message_id)) {
            echo 'EVENT_RECEIVED';
            return;
        }

        if ($mensaje['type'] !== 'text' || empty($mensaje['text']['body'])) {
            $this->Whatsapp_model->guardar_mensaje($telefono, 'user', '[mensaje no soportado: ' . $mensaje['type'] . ']', $wa_message_id);
        ...
        $this->Whatsapp_model->guardar_mensaje($telefono, 'user', $texto_cliente, $wa_message_id);

        $historial = $this->Whatsapp_model->historial_reciente($telefono, $this->historial_limite);
    ...
    private function _responder($telefono, $texto) {
        $wa_message_id = $this->_enviar_mensaje_whatsapp($telefono, $texto);
        $this->Whatsapp_model->guardar_mensaje($telefono, 'assistant', $texto, $wa_message_id);
    }
    ...
    public function historial() {
        ...
        $this->load->view('admin/whatsapp_historial', $data);
    }

    public function historial_json() {
        $this->_check_admin();

        $conversaciones = $this->Whatsapp_model->conversaciones_recientes();
    ...
    public function mensajes($telefono) {
        ...
        $mensajes = $this->Whatsapp_model->mensajes_por_telefono($telefono);
